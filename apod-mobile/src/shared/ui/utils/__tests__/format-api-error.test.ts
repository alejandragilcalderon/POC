...
describe("format-api-error", () => {
  it("friendlyHttpStatus maps known codes", () => {
    ...
    expect(friendlyHttpStatus(500)).toMatch(/Something went wrong on the server/);
  });
  ...
  it("friendlyNetworkMessage explains unreachable API", () => {
    expect(friendlyNetworkMessage("Network request failed")).toMatch(/couldn’t reach the service/);
  });
});
